...
对$\boldsymbol{a}$求导，$\frac{\partial \mathrm{mse}(\hat{\theta})}{\partial \boldsymbol{a}}=2 \boldsymbol{R}_{\boldsymbol{y} \boldsymbol{y}} \boldsymbol{a}-2 \boldsymbol{R}_{\boldsymbol{y} \theta}$，令其为零，得到$a=R_{y y}{ }^{-1} R_{y \theta}$。
<center>
$$
\begin{aligned}
