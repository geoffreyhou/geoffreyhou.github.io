...
令其为零，得到$a[N]=\mathrm{E}[\theta]-\sum_{i=0}^{N-1} a[i] \mathrm{E}[y[i]]$。这样$\operatorname{mse}(\hat{\theta})$可以携程矩阵形式
<center>
...
对$a$求导，$\frac{\partial \mathrm{mse}(\hat{\theta})}{\partial \boldsymbol{a}}=2 \boldsymbol{R}_{\boldsymbol{y} \boldsymbol{y}} \boldsymbol{a}-2 \boldsymbol{R}_{\boldsymbol{y} \theta}$，令其为零，得到$a=R_{y y}{ }^{-1} R_{y \theta}$。
<center>
$$
\begin{aligned}
