...
对$\boldsymbol{a}$求导，$\frac{\partial \mathrm{mse}(\hat{\theta})}{\partial \boldsymbol{a}}=2 \boldsymbol{R}_{\boldsymbol{y} \boldsymbol{y}} \boldsymbol{a}-2 \boldsymbol{R}_{\boldsymbol{y} \theta}$，令其为零，得到$\boldsymbol{a}=\boldsymbol{R_{yy}}^{-1}\boldsymbol{R_{y\theta}}$。
<center>
$$
\begin{aligned}
