...
其中$\boldsymbol{y}$是$N \times 1$的观测数据矢量，$\boldsymbol{H}$是已知$N \times p$矩阵，$\boldsymbol{\theta}$是$p \times 1$的随机矢量，先验概率PDF满足$\mathcal{N}\left(\boldsymbol{\mu},\boldsymbol{C}_\theta\right)$，$\boldsymbol{n}$是$N \times 1$的噪声矢量，PDF满足$\mathcal{N}\left(\boldsymbol{0},\boldsymbol{C}_\theta\right)$，并且和$\boldsymbol{\theta}$无关。
