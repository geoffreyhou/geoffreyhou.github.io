...
$\widehat{\boldsymbol{\theta}}=\mathrm{E}[\boldsymbol{\theta}]+\boldsymbol{R}_{\boldsymbol{\theta} \boldsymbol{y}} \boldsymbol{R}_{\boldsymbol{y} \boldsymbol{y}}{ }^{\mathbf{1}}(\boldsymbol{y}-\mathrm{E}[\boldsymbol{y}])$是矢量参数LMMSE的原始表达式，仅限定估计量是线性的。
利用线性贝叶斯的结论：
<center>
$$
