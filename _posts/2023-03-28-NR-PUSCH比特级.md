...
用$M_{\mathrm{sc}}^{\mathrm{UL}-\mathrm{SCH}}(l)=\left|\Phi_l^{\mathrm{UL}-\mathrm{SCH}}\right|$表示集合$\left|\Phi_l^{\mathrm{UL}-\mathrm{SCH}}\right|$中的元素个数，$\Phi_l^{\mathrm{UL}-\mathrm{SCH}}(j)$表示第$j$个元素。

用$\Phi_l^{\mathrm{UCI}}$表示符号$l$上可以给UCI用的RE的集合，以$k$的升序表示，$l=0,1,2, \ldots, N_{\text {symb,all }}^{\mathrm{PUSCH}}-1$。

用$M_{\mathrm{sc}}^{\mathrm{UCI}}(l)=\left|\Phi_l^{\mathrm{UCI}}\right|$表示集合中的元素个数。$\Phi_l^{\mathrm{UCI}}(j)$表示第$j$个元素。
