...
如果portNum=1或者reportQuantity=CRI-RI-CQI或者CRI-RI-i1-CQI，那么$n=1$。



RI PMI CQI LI

通过一级码本搜索得到秩$r$下的一级码本相关的最优参数
$$
\boldsymbol{\alpha}_r=\left(\hat{l}, \widehat{m}, \hat{i}_{1,3}\right)_r
$$
$I$表示比特互信息，$q$表示调制方式，$o$表示调制阶数，$Q$表示CQI值，$I_S$表示符号互信息，$L$表示层指示。



先计算出秩$r$下的一级码本相关的最优参数$\left(\hat{l}, \hat{m}, \hat{i}_{1,3}\right)_r$。，然后得到第$j$个子带第$i$层的信噪比$\gamma_{r, \boldsymbol{\alpha}_r, n, j}^i$。

根据信噪比得到不同调制方式下的比特互信息。

把不同秩下的子带比特互信息累加得到子带比特互信息：
$$
I_{r, \boldsymbol{\alpha}_r, n, j}^q=\frac{1}{r} \sum_{i=1}^r I_{r, \boldsymbol{\alpha}_r, n, j}^{q, i}
$$
对不同子带比特互信息累加，得到宽带比特互信息：
$$
I_{r, \alpha_r, n}^q=\frac{1}{N_{s b}} \sum_{j=0}^{N_{s b}-1} I_{r, \alpha_r, n, j}^q
$$
宽带比特互信息最大的$n$值就是在秩$r$，调制方式$q$下，与二级码本指示相关的最优$n$值：
$$
n_r^q=\arg \max _{\{n\}}\left(I_{r, \alpha_r, n}^q\right)
$$
得到最优$n$值后，可以反过来得到这个值对应的宽带比特互信息$I_{r, \alpha_r, n_r^q}^q$。这样就可以查表得到最优宽带CQI $Q_r$，一级调制方式$q_r$。





如果是1 layer，并且*codebookMode=1*：$i_{1,1}$对应$l$。$i_{1,2}$对应$m$。$i_{2}$对应$n$。

如果是1 layer，并且*codebookMode=2*：$2i_{1,1}$或$2i_{1,1}+1$对应$l$。$2i_{1,2}$或$2i_{1,2}+1$对应$m$。$i_{2}={0,1,\cdots, 15}$对应$n={0,1,2,3}$。
