...
如果要支持分页，需要在前面加上这一段。但是要把里面XX的位置替换成{%，因为不知道为什么用{%的话Github Pages编译会不过。
```Latex
\makeatletter
\newenvironment{breakablealgorithm}
  XX \begin{breakablealgorithm}
   \begin{center}
     \refstepcounter{algorithm}% New algorithm
     \hrule height.8pt depth0pt \kern2pt% \@fs@pre for \@fs@ruled
     \renewcommand{\caption}[2][\relax] XX Make a new \caption
